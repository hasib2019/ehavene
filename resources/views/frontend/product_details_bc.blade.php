                ...
                <div class="col-xl-3 d-none d-xl-block">
                    ...
                    <div class="seller-info-box mb-3">
                        <div class="sold-by position-relative">
                            @if ($product->added_by == 'seller' && \App\Models\BusinessSetting::where('type', 'vendor_system_activation')->first()->value == 1 && $product->user->seller->verification_status == 1)
                    ...
                    <div class="seller-category-box bg-white sidebar-box mb-3">
                        ...
                        <div class="box-content">
                            <div class="category-accordion">
                                @foreach (\App\Models\Product::where('user_id', $product->user_id)->select('category_id')->distinct()->get() as $key => $category)
                                    <div class="single-category">
                                        ...
                                        {{ App\Models\Category::findOrFail($category->category_id)->name }}
                                        </button>

                                        <div id="category-{{ $key }}" class="collapse">
                                            @foreach (\App\Models\Product::where('user_id', $product->user_id)->where('category_id', $category->category_id)->select('subcategory_id')->distinct()->get() as $subcategory)
                                                <div class="single-sub-category">
                                                    ...
                                                    {{ App\Models\SubCategory::findOrFail($subcategory->subcategory_id)->name }}
                                                    </button>
                                                    <div id="subCategory-{{ $subcategory->subcategory_id }}" class="collapse show">
                                                        <ul class="sub-sub-category-list">
                                                            @foreach (\App\Models\Product::where('user_id', $product->user_id)->where('category_id',            $category->category_id)->where('subcategory_id', $subcategory->subcategory_id)->select('subsubcategory_id')->distinct()->get() as $subsubcategory)
                                                                <li><a href="{{ route('products.subsubcategory', $subsubcategory->subsubcategory_id) }}">{{ App\Models\SubSubCategory::findOrFail($subsubcategory->subsubcategory_id)->name }}</a></li>
                                                            @endforeach
                                                    </div>
                                                </div>
                                            @endforeach
                    ...
                    <div class="seller-top-products-box bg-white sidebar-box mb-3">
                        ...
                        <div class="box-content">
                            @foreach (\App\Models\Product::where('user_id', $product->user_id)->orderBy('num_of_sale', 'desc')->limit(4)->get() as $key => $top_product)
