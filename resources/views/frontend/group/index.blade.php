                        ...
                        @foreach ($groupdlt as $group)
                           <div class="column">
                            ...
                            <div class="container">
                                ...
                                @if($group->cid==$group->group_by && $group->cid==Auth::user()->cid)
                               @php
                                   $data= \app\Models\User::where('group_by','=', Auth::user()->group_by)->count('group_by');
                               @endphp
                               @if($data==1)
                               <p><a href="{{route('group.changeyourself', $group->cid)}}"><button class="button">Remove Yourself</button></a></p>
                                @else
                                <p><a href="{{route('group.change')}}"><button class="button">change</button></a></p>
                                @endif
                                @elseif($group->cid==Auth::user()->cid && $group->cid!=$group->group_by )
                                <p><a href="{{route('group.changeyourself', $group->cid)}}"><button class="button">Remove Yourself</button></a></p>

                                @elseif($group->group_by==Auth::user()->cid)
                                <p><a href="{{route('group.changeyourself', $group->cid)}}"><button class="button">Remove User</button></a></p>
                                @endif
                            </div>
                            </div>
                        </div>
