            ...
            <div class="row">
                        ...
                        <div class="box-content">
                            <div class="category-accordion">
                                @php
                                    $brands = array();
                                @endphp
                                @foreach (\App\Models\Product::where('user_id', $shop->user->id)->select('category_id')->distinct()->get() as $key => $category)
                                    <div class="single-category">
                                        ...
                                        <div id="category-{{ $key }}" class="collapse">
                                            @foreach (\App\Models\Product::where('user_id', $shop->user->id)->where('category_id', $category->category_id)->select('subcategory_id')->distinct()->get() as $subcategory)
                                                <div class="single-sub-category">
                                                    ...
                                                    <div id="subCategory-{{ $subcategory->subcategory_id }}" class="collapse">
                                                        <ul class="sub-sub-category-list">
                                                            @foreach (\App\Models\Product::where('user_id', $shop->user->id)->where('category_id',            $category->category_id)->where('subcategory_id', $subcategory->subcategory_id)->select('subsubcategory_id')->distinct()->get() as $subsubcategory)
                                                                @php
                                                                    $subsubcategory = App\Models\SubSubCategory::findOrFail($subsubcategory->subsubcategory_id);
                ...
                <div class="col-xl-9">
                    ...
                    <div class="product-list row">
                        @php
                            if (!isset($type)){
                                $products = \App\Models\Product::where('user_id', $shop->user->id)->where('published', 1)->orderBy('created_at', 'desc')->paginate(6);
                            }
                            elseif ($type == 'top_selling'){
                                $products = \App\Models\Product::where('user_id', $shop->user->id)->where('published', 1)->orderBy('num_of_sale', 'desc')->paginate(6);
                            }
                            elseif ($type == 'all_products'){
                                $products = \App\Models\Product::where('user_id', $shop->user->id)->where('published', 1)->paginate(6);
                            }
                        @endphp
