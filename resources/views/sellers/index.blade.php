        ...
        <table class="table table-striped table-bordered demo-dt-basic" cellspacing="0" width="100%">
            ...
            <tbody>
                @foreach($sellers as $key => $seller)
                    ...
                    <tr>
                        <td>{{$key+1}}</td>
                        <td>{{isset($seller->user->name)?$seller->user->name:''}}</td>
                        <td>{{ isset($seller->user->email)?$seller->user->email:'' }}</td>
                        <td>
                        ...
                        <td>{{ \App\Models\Product::where('user_id', isset($seller->user->id)?$seller->user->id:'')->count() }}</td>
                        <td>
                        ...
                        <td>
                            <div class="btn-group dropdown">
                                ...
                                <ul class="dropdown-menu dropdown-menu-right">
                                    ...
                                    <li>
                                        @if ($seller->verification_status == 1)
                                            <a href="{{ route('sellers.reject', $seller->id) }}">{{__('Reject')}}</a>
                                            @else
                                            <a href="{{ route('sellers.approve', $seller->id) }}">{{__('Accept')}}</a>
                                        @endif
